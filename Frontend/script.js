...
products.forEach(p => {
    ...
    card.innerHTML = `
     ...
     onerror="this.src='https://res.cloudinary.com/dc7udh4me/image/upload/v1/sample.jpg'">
        <h3>${safeName}</h3>
        <p>₹${p.price}</p>
        <button class="btn btn-primary" onclick="addToCart(${p.id}); event.stopPropagation();">
            Add to Cart
        </button>
    `;
    ...
    products.forEach(p => {
        ...
        <td>
            <button class="btn btn-success"
                onclick="editProduct(${p.id}, '${safeName}', ${p.price})">
                Edit
            </button>
        </td>
...
async function updateQuantity(id, newQty, btn) {
    ...
    try {
        ...
        // 🔥 ADD TOAST HERE (optional but controlled)
...
function showToast(msg, type = "info") {
    ...
    toast.className = "toast " + type;  // 🔥 important
    toast.innerText = msg;
...
async function loadOrders() {
    ...
    try {
        const res = await authFetch(`${API_URL}/orders/${encodeURIComponent(user.email)}`);
